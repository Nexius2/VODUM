...
class FakeDb:
    ...
    def query_one(self, sql, params=()):
        if "FROM admin_sessions WHERE token_hash" in sql:
    ...
    def execute(self, sql, params=()):
        ...
        if "INSERT INTO admin_sessions" in sql:
            self.admin_session = {"id": 1, "expires_at": params[2]}
...
class PlexAuthRouteTests(unittest.TestCase):
    ...
    def test_fresh_install_offers_plex_before_leaving_local_account_step(self):
        ...
        self.assertNotIn('must always expose the restore/new', setup_route)
        self.assertIn('step = _display_step(db, settings, state)', setup_route)
    ...
    def test_global_auth_guard_does_not_intercept_fresh_wizard_plex_callback(self):
        auth_guard = (APP_DIR / "core" / "global_access_guard.py").read_text(encoding="utf-8")
        access_policy = (APP_DIR / "core" / "route_access_policy.py").read_text(encoding="utf-8")
        self.assertIn('"/auth/plex/wizard-link"', access_policy)
        self.assertIn('"/auth/plex/link/callback"', access_policy)
        self.assertIn('"/auth/plex/link/confirm"', access_policy)
        self.assertIn("classify_route_path(request.path)", auth_guard)
    ...
    def test_plex_login_requires_exact_linked_subject_and_skips_vodum_totp(self):
        ...
        client = self.app.test_client()
        with client.session_transaction() as browser_session:
            browser_session["attacker_fixed_marker"] = "must-not-survive"
        ...
        with client.session_transaction() as browser_session:
            ...
            self.assertNotIn("attacker_fixed_marker", browser_session)
    ...
    def test_pending_plex_totp_proof_expires_and_is_consumed(self):
        client = self.app.test_client()
        with client.session_transaction() as browser_session:
            browser_session[plex_auth.PENDING_LOGIN_KEY] = {
                "identity_id": 7,
                "subject": "plex-42",
                "started_at": 1,
            }

        with patch.object(plex_auth.time, "time", return_value=302):
            response = client.post(
                "/auth/plex/login/totp", data={"totp_code": "123456"}
            )
        ...
        with client.session_transaction() as browser_session:
            self.assertNotIn(plex_auth.PENDING_LOGIN_KEY, browser_session)
            self.assertNotIn("vodum_principal", browser_session)
