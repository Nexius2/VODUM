...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="vodum-runtime-", ignore_cleanup_errors=True) as tmp:
        ...
        connection.execute(
            ...
            ) VALUES (1, 'plex', 'runtime-plex-user', 'Runtime Plex',
                      'plex@example.test', 1)
            """
        )
        connection.commit()
        ...
        from routes import plex_auth as plex_auth_routes

        class _FakePlexAuthClient:
            def create_pin(self):
                return SimpleNamespace(id=42, code="runtime-code")

            def build_authorization_url(self, _pin, _callback):
                return "https://app.plex.tv/auth#runtime-smoke"

        original_plex_client = plex_auth_routes._client
        plex_auth_routes._client = lambda _db: _FakePlexAuthClient()
        try:
            plex_login_client = app.test_client()
            response = plex_login_client.get("/login")
            assert response.status_code == 200
            with plex_login_client.session_transaction() as plex_session:
                plex_csrf = plex_session["_csrf_token"]
            response = plex_login_client.post(
                "/auth/plex/login",
                data={"_csrf_token": plex_csrf},
                follow_redirects=False,
            )
            assert response.status_code == 302
            assert response.headers["Location"].startswith("https://app.plex.tv/")
        finally:
            plex_auth_routes._client = original_plex_client
        ...
        for rule in app.url_map.iter_rules():
            if "GET" not in rule.methods or rule.endpoint == "static":
                continue
            path = _route_path(rule)
            if path in public_paths or path.startswith(("/static", "/login/artwork/", "/auth/plex/login")):
                continue
        ...
        assert response.status_code == 200
        response = anonymous.get("/login/artwork/poster", follow_redirects=False)
        assert response.status_code != 302, (
            "Public login artwork was redirected through the authentication guard"
        )
        ...
        settings_form_end = settings_html.index("</form>", settings_form_start)
        plex_form_start = settings_html.index('id="plex-auth-link-form"')
        assert plex_form_start > settings_form_end, (
            "The Plex form must be outside the main settings form; nested forms "
            "cause the Plex button to save settings instead"
        )
        assert 'form="plex-auth-link-form"' in settings_html
        plex_card_html = settings_html.split(
            'data-testid="plex-auth-settings-card"', 1
        )[1].split('data-testid="encryption-key-status-card"', 1)[0]
        assert 'name="current_password"' not in plex_card_html
