...
def load_portal_profile(db, vodum_user_id: int) -> dict | None:
    row = db.query_one(
        ...
        SELECT id,username,firstname,lastname,email,second_email,status,created_at,
               preferred_language,notifications_order_override
...
def update_portal_profile(db, vodum_user_id: int, values: dict, *, notifications_can_override=True) -> None:
    db.execute(
        "UPDATE vodum_users SET firstname=?,lastname=?,second_email=?,preferred_language=?,"
        "notifications_order_override=CASE WHEN ?=1 THEN ? ELSE notifications_order_override END WHERE id=?",
        (
            values.get("firstname"), values.get("lastname"),
            values.get("second_email"), values.get("preferred_language"),
            int(bool(notifications_can_override)), values.get("notifications_order_override"), int(vodum_user_id),
        ),
    )
