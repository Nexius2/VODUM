...
def register(app):
    ...
    def portal_profile():
        ...
        setting = dict(db.query_one(
            "SELECT user_notifications_can_override,portal_local_auth_enabled,portal_plex_auth_enabled,portal_jellyfin_auth_enabled FROM settings WHERE id=1"
        ) or {})
        ...
        return render_template(
            ...
            notifications_can_override=True,
    ...
    def portal_profile_save():
        ...
        if errors:
            for error in errors:
                flash(error, "error")
            return redirect(url_for("portal_profile"))
        update_portal_profile(get_db(), int(g.auth_principal["vodum_user_id"]), values, notifications_can_override=True)
    ...
    def portal_support_message_send():
        ...
        enabled = db.query_one("SELECT portal_quick_messages_enabled,contact_email,admin_email FROM settings WHERE id=1") or {}
        ...
        if recipient:
            send_email_via_settings(recipient, "VODUM - nouveau message utilisateur", f"Un nouveau message de {conversation.get('username') or conversation.get('email') or 'un utilisateur'} est disponible dans Communications > Messages.")
